...
describe "Kernel.lambda" do
  ...
  it "returns from the lambda itself, not the creation site of the lambda" do
    ...
    test
    NATFIXME 'it returns from the lambda itself, not the creation site of the lambda', exception: SpecFailedException do
  ...
  it "treats the block as a Proc when lambda is re-defined" do
    ...
    NATFIXME 'it treats the block as a Proc when lambda is re-defined', exception: SpecFailedException do
  ...
  context "when called without a literal block" do
    ...
    it "doesn't warn when proc is lambda" do
      NATFIXME "it doesn't warn when proc is lambda", exception: ArgumentError, message: 'the lambda method requires a literal block' do
